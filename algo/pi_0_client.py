...
class TaskConfig:
    ...
    object_to_manipulate: str = "pull_drawer"  # banana, carrot, oranges , green pepper
...
class RobotClient:
    ...
    def inference(self, img_resized, wrist_img_resized, gripper_state, gripper_width, language_instruction):
        state_with_action = np.concatenate([
            gripper_state, 
            [gripper_width] # 添加上一步action的夹爪状态
        ], axis=-1)
        print("state:", state_with_action)
        obs = {
            'observations/images/image': img_resized,
            'observations/images/wrist_image': wrist_img_resized,
            'observations/state':state_with_action,
        ...
        print(f"task description is {obs['prompt']}")
        actions = self.policy_client.infer(obs)["actions"].astype(np.float32)
        print("inferred actions:", actions.shape, actions.dtype)
        return actions
...
class shw_franka:
    def __init__(self, robot_cfg, task_cfg):
        ...
        if self.object_to_manipulate == "banana":
            self.language_instruction = "Pick up the banana and place it into the red bowl"
        elif self.object_to_manipulate == "carrot":
            self.language_instruction = "Pick up the carrot and place it into the red bowl"
        elif self.object_to_manipulate == "orange":
            self.language_instruction = "Pick up the orange and place it into the red bowl"
        elif self.object_to_manipulate == "green pepper":
            self.language_instruction = "Pick up the green pepper and place it into the red bowl"
        elif self.object_to_manipulate == "banana_goal":
            self.language_instruction = "Pick up the banana and place it into the yellow bowl"
        elif self.object_to_manipulate == "carrot_goal":
            self.language_instruction = "Pick up the carrot and place it into the yellow bowl"
        elif self.object_to_manipulate == "orange_goal":
            self.language_instruction = "Pick up the orange and place it into the yellow bowl"
        elif self.object_to_manipulate == "pick_drawer":
            self.language_instruction = "Pick up the object in the drawer and place it into the red bowl"
        elif self.object_to_manipulate == "pull_drawer":
            self.language_instruction = "Pull the drawer open"
        else:
    ...
    def prepare_observation(self, images, state):
        ...
        return img_resized, wrist_img_resized, qpos, gripper_width
    ...
    def get_action(self, control_hz: int = 10):
        ...
        while not self._stop_event.is_set():
            ...
            img_resized, wrist_img_resized, qpos, gripper_width = self.prepare_observation(images, state)
            
            action_list = self.client.inference(img_resized, wrist_img_resized, qpos, gripper_width, language_instruction)
            print("raw action shape", action_list.shape)
            action_list = self.process_action(action_list)
            processed_action_list = self.merge_action_sequence(action_list, 4)
            print("processed action shape:", processed_action_list.shape)  
                
            if processed_action_list is None:
                print("No action")
                continue
            elif isinstance(processed_action_list[0], (list, tuple, np.ndarray)):
                for i in range(1):
                    try:
                ...
                try:
                    self.move(list(processed_action_list))
                except Exception as e:
                    print(f"[get_action] apply action failed: {e}")
