...
class TaskConfig:
    ...
    object_to_manipulate: str = "banana"  # banana, carrot, orange
...
class shw_franka:
    def __init__(self, robot_cfg, task_cfg):
        ...
        elif self.object_to_manipulate == "cube1":
            self.language_instruction = "Stack the red cube on the top of the green cube."
        elif self.object_to_manipulate == "cube2":
            self.language_instruction = "Place the blue cube on the top of the red cube."
        elif self.object_to_manipulate == "pot":
            self.language_instruction = "Put the pot on the shelf."
        elif self.object_to_manipulate == "can":
            self.language_instruction = "Place the orange can on the shelf."
        else:
    ...
    def normalize_gripper_action(self, action: np.ndarray, binarize: bool = True) -> np.ndarray:
        ...
        if binarize:
            # 二值化：>=0.5为关(1)，<0.5为开(0)
            normalized_action[:, -1] = np.where((1 - normalized_action[:, -1]) >= 0.9, 1, 0)
    ...
    #     # 合并欧拉角 - 使用旋转矩阵，指定XYZ顺序
    #     merged_rot = R.identity().as_matrix()
        
    #     for action in actions_chunk:
    #         # 获取当前动作的欧拉角（假设在索引3,4,5）
    #         euler_angles = action[3:6]
    #         # 将欧拉角转换为旋转矩阵，使用XYZ顺序
    #         rot = R.from_euler('XYZ', euler_angles, degrees=False)
    #         action_rot = rot.as_matrix()
    #         # 累乘旋转矩阵
    #         merged_rot = merged_rot @ action_rot

    #     # 转换回欧拉角，保持XYZ顺序
    #     merged_quat = R.from_matrix(merged_rot).as_quat()
        
    #     merged_action = np.concatenate([merged_pos, merged_quat, [merged_gripper]])
    #     return merged_action
    ...
    def merge(self, actions_chunk):
        ...
        # 合并夹爪状态
        merged_gripper = actions_chunk[-1, -1]
        ...
        for action in actions_chunk:
            ...
            # 将欧拉角转换为旋转（假设是弧度）
            # 如果您的数据集中欧拉角是度，需要转换：euler_angles = np.radians(euler_angles)
            rot = R.from_euler('xyz', euler_angles, degrees=False)
            
            # 工具坐标系增量：右乘 merged_rot = merged_rot * rot
            # 注意：使用 * 运算符而不是 @，因为 Rotation 对象不支持 @
            merged_rot = merged_rot * rot
        
        # 将合并后的旋转转换为四元数 [x, y, z, w]
        merged_quat = merged_rot.as_quat()
        
        # 创建合并后的动作
        merged_action = np.zeros(8)
        merged_action[:3] = merged_pos
        merged_action[3:7] = merged_quat
        merged_action[7] = merged_gripper
    ...
    def get_action(self, control_hz: int = 10):
        ...
        while not self._stop_event.is_set():
            ...
            print("processed action", processed_action_list)
