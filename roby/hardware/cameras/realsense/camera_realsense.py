...
from roby.hardware.cameras.camera import Camera
from roby.hardware.cameras.configs import ColorMode
from roby.hardware.cameras.utils import get_cv2_rotation
from roby.hardware.cameras.realsense.configuration_realsense import RealSenseCameraConfig

logger = logging.getLogger(__name__)


@dataclass
class RealSenseCameraOutput(BaseHardwareOutput):
    
    color: Optional[np.ndarray] = None
    depth: Optional[np.ndarray] = None

    def _image_keys(self) -> List[str]:
        """Returns the keys for image data in this output."""
        return ["color", "depth"]
...
class RealSenseCamera(Camera):
    ...
    def __init__(self, config: RealSenseCameraConfig):
        ...
        self.rs_profile: Optional[rs.pipeline_profile] = None

        self.thread: Optional[Thread] = None
        self.stop_event: Optional[Event] = None
        self.frame_buffer: FIFOBuffer = FIFOBuffer(maxsize=config.buffer_size)
    ...
    def find_cameras() -> List[Dict[str, Any]]:
        ...
        devices.sort(key=lambda d: d.get_info(rs.camera_info.serial_number))
    ...
    def read_depth(self, timeout_ms: int = 200) -> RealSenseCameraOutput:
        """
        ...
        timestamp = frame.get_timestamp() if frame else None
        ...
        return RealSenseCameraOutput(timestamp=timestamp, depth=depth_map_processed)

    def read_color(self, color_mode: Optional[ColorMode] = None, timeout_ms: int = 200) -> RealSenseCameraOutput:
        """
        ...
        return RealSenseCameraOutput(timestamp=timestamp, color=color_image_processed)
    
    def read(self, color_mode: Optional[ColorMode] = None, timeout_ms: int = 200) -> RealSenseCameraOutput:
        """
        Reads a single frame (color and depth) synchronously from the camera.
        ...
        return RealSenseCameraOutput(timestamp=timestamp, color=color_image_processed, depth=depth_map_processed)
    ...
    def _read_loop(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                self.frame_buffer.put(color_image)
    ...
    def async_read(self, timeout_ms: float = 200) -> RealSenseCameraOutput:
        """
        ...
        frame = self.frame_buffer.queue[-1] if not self.frame_buffer.empty() else None
...
if __name__ == "__main__":
    ...
    config = RealSenseCameraConfig(serial_number_or_name="216322072028", fps=15, height=720, width=1280, use_depth=False)
    camera = RealSenseCamera(config)
    camera.connect()
    ...
    while True:
        ...
        if frame is not None:
            print(f"Captured frame shape: {frame.color.shape if frame.color is not None else 'No color frame'} | FPS: {camera.reading_fps:.2f}")
        else:
